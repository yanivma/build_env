...
declare -A hostap_brnach["r8"]="android_jb"
...
function compat-wireless()
{
	...
	then
		cd ${WORK_SPACE}
		git clone "${wl18xx_download_target[$BUILD_VERSION]}" ${GIT_TREE} -b "${wl18xx_branch[$BUILD_VERSION]}"
		git clone "${compat_download_target[$BUILD_VERSION]}" ${GIT_COMPAT_TREE} -b "${compat_branch[$BUILD_VERSION]}"
		git clone "${compatwireless_download_target[$BUILD_VERSION]}" compat-wireless -b "${compatwireless_branch[$BUILD_VERSION]}"
	fi
	...
	then
		...
		make KLIB_BUILD=${KLIB_BUILD} KLIB=${ROOTFS} -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1        
	fi
	...
	then
		...
		sh ./scripts/admin-refresh.sh
		./scripts/driver-select wl18xx        
		make KLIB_BUILD=${KLIB_BUILD} KLIB=${ROOTFS} install-modules -j$(egrep '^processor' /proc/cpuinfo | wc -l)
	fi
...
function crda ()
{
	...
	then
		cd ${WORK_SPACE}/crda-1.1.1
		make USE_OPENSSL=1 all_noverify -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
	fi
	if [ x"$stage" = "xinstall"  -o x"$stage" = "xall" ]
	then
		cd ${WORK_SPACE}/crda-1.1.1
...
function iw ()
{
	...
	then
		cd ${WORK_SPACE}/iw
		make -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
	fi

	if [ x"$stage" = "xinstall"  -o x"$stage" = "xall" ]
	then
		cd ${WORK_SPACE}/iw
		DESTDIR=${ROOTFS} make install -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
	fi
	if [ x"$stage" = "xclean" ]
	then
		cd $WORK_SPACE/iw
...
function libnl ()
{
	...
	then
		cd ${WORK_SPACE}/libnl-2.0
		make install -j$(egrep '^processor' /proc/cpuinfo | wc -l) ||  exit 1
	fi
	if [ x"$stage" = "xclean" ]
	then
		cd $WORK_SPACE/libnl-2.0
...
function openssl ()
{
	...
	then
		...
		unzip -u ${WORK_SPACE}/Openssl-1.0.0d-new-compilation-target-for-configure.zip || exit 1
	...
	then
		cd ${WORK_SPACE}/openssl-1.0.0d
		make install_sw -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
	fi
	if [ x"$stage" = "xclean" ]
	then
		cd ${WORK_SPACE}/openssl-1.0.0d
...
function ti-utils ()
{
	...
	if [ x"$stage" = x"download" -o x"$stage" = "xall" ]
	then
		git clone "${ti_utils_download_target[$BUILD_VERSION]}" 18xx-ti-utils -b "${ti_utils_branch[$BUILD_VERSION]}"
	fi

	if [ x"$stage" = "xbuild" -o x"$stage" = "xall" ]
	then
		cd ${WORK_SPACE}/18xx-ti-utils
		NFSROOT=${ROOTFS} make -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
	fi
	if [ x"$stage" = "xinstall" -o x"$stage" = "xall" ]
	then
		cd ${WORK_SPACE}/18xx-ti-utils
		#NFSROOT=${ROOTFS} make install -j$(egrep '^processor' /proc/cpuinfo | wc -l) || exit 1
		if [ ! -x calibrator ]
		then
...
function wl18xx-firmware()
{
	...
	if [ x"$stage" = x"download" -o x"$stage" = "xall" ]
	then
		git clone "${wl18xx_fw_download_target[$BUILD_VERSION]}" wl18xx_fw -b "${wl18xx_fw_branch[$BUILD_VERSION]}"
	fi
...
function hostap()
{
	...
	then
		cd ${WORK_SPACE}
		git clone "${hostap_download_target[$BUILD_VERSION]}" hostap -b "${hostap_branch[$BUILD_VERSION]}"
		hostap_patching
	fi
	if [ x"$stage" = "xbuild" -o x"$stage" = "xall" ]
	then
...
function wpa_supplicant ()
{
	...
	then
		...
		cp ${BASE_SCRIPTS_DIR}/conf/wpa_supplicant/wpa_supplicant.conf ${ROOTFS}/etc/ 
	fi
	cd $WORK_SPACE
}
...
function wlconf ()
{
	...
	then
		cd ${WORK_SPACE}/18xx-ti-utils/wlconf
		NFSROOT=${ROOTFS} make -j$(egrep '^processor' /proc/cpuinfo | wc -l) WLCONF_DIR='-DWLCONF_DIR=\"/usr/sbin/wlconf/\"' || exit 1
	fi
	if [ x"$stage" = x"install" -o x"$stage" = "xall" ]
	then
		cd ${WORK_SPACE}/18xx-ti-utils/wlconf
		...
		cp ${BASE_SCRIPTS_DIR}/conf/wlconf/* ${ROOTFS}/home/root/scripts/wlconf
...
then
	if [ x"$1" != x"all" -a x"$1" != x"clean-all" -a x"$1" != x"download-all" ]
	then
		usage
		exit 1
	else
